...
mod month_day {
    ...
    fn month_day_display() {
        ...
        expected.iter().enumerate().for_each(|(prev_day, display)| {
            let month_day = MonthDay::new((prev_day + 1).try_into().unwrap()).unwrap();
            assert_eq!(format!("{}", month_day), display.to_string());
        });
    }
}
